  ...
  items: [
    {
      type: 'radio',
      options: [
        { text: 'Мальчик', value: 'icon-sex-1' },
        { text: 'Девочка', value: 'icon-sex-2' },
        { text: 'Все', value: 'all' },
      ],
      group: [
        {
...
const menuSpecialCapture = new Menu({
  title: 'Поймать Яркокрылов и Семантов',
  text: 'Указывать кого ловить через запятую.',
  items: [
    {
      type: 'input',
      text: 'Яркокрылы:',
...
const menuWeather = new Menu({
  ...
  text: 'Останавливается только если идёт прокачка или ловля.',
  items: [
    {
      type: 'radio',
      options: [
        ...
        { text: 'Град и Песчаная буря', value: 'w3,w4' },
      ],
      group: [
        {
...
//   constructor() {
//     this.arrIgnore = ['.divChatArea', '.post.ad']
...
//             if (!post) continue
//             if (this.arrIgnore.some((sel) => post.matches(sel))) continue
//             console.log(el)
...
//     obs.observe(document.querySelector('#divChatAreas'), {
//       childList: true,
//       subtree: true,
//     })
//   }
...
//     if (userTwo !== userName) {
//       if (!(await this.calcTime(post))) return

//       soundController.play('message')
//       showNotification('Сообщение', 'Новое сообщение от: ' + userTwo)

//       if (settings.get('antiBotEnable') === true) {
//         if (settings.get('variableAntiBot') === 'stop') bot.stop()
//         if (settings.get('variableAntiBot') === 'pause') bot.pause()
//       }
//     }
//   }
...
//   }
//   async calcTime(el) {
//     // Если есть месяцы то false
//     const [nHour, nMin] = document.querySelector('#divDockClock').textContent.trim().split(':').map(Number)
//     const [mHour, mMin] = el.querySelector('.time').textContent.trim().split(':')?.map(Number)

//     const nowTime = nHour * 60 + nMin
//     const messageTime = mHour * 60 + mMin

//     const diff = nowTime - messageTime
